...
EV_KEYS = ("ev1", "ev2")
SPLIT_MODES = ("strict_all", "strict_energy_only", "strict_none")
...
def _normalize_signature(signature: tuple, decimals: int = 8) -> tuple:
    max_abs = max((abs(float(value)) for value in signature), default=0.0)
    if max_abs == 0.0:
        return tuple(0.0 for _ in signature)
    return tuple(round(float(value) / max_abs, decimals) for value in signature)


def _signature_key(profile_name: str, signature: tuple) -> tuple:
    # Continuous profiles are matched by normalized shape (scale-invariant).
    if profile_name in {"base_load", "pv_gen"}:
        return _normalize_signature(signature)
    # Categorical EV status profiles are matched exactly.
    return signature


def _load_has_pv_flags() -> dict[int, bool]:
    with sqlite3.connect(Config.SQLITE_PATH) as conn:
        df = pd.read_sql_query("SELECT player_id, has_pv FROM player_pv_bess", conn)

    df["player_id"] = pd.to_numeric(df["player_id"], errors="coerce").fillna(0).astype(int)
    df["has_pv"] = pd.to_numeric(df["has_pv"], errors="coerce").fillna(0).astype(int)
    return {int(row["player_id"]): bool(int(row["has_pv"])) for _, row in df.iterrows()}
...
def _profile_names_for_mode(mode: str) -> set[str]:
    if mode == "strict_all":
        return {"base_load", "pv_gen", "ev1_status", "ev2_status"}
    if mode == "strict_energy_only":
        return {"base_load", "pv_gen"}
    if mode == "strict_none":
        return set()
...
def _build_duplicate_groups(
    profiles_by_name: dict[str, dict[int, tuple]],
    mode: str,
) -> tuple[list[int], dict[str, list[list[int]]]]:
    active_profiles = _profile_names_for_mode(mode)
    ...
    has_pv_by_household = _load_has_pv_flags()

    for profile_name, profile_map in profiles_by_name.items():
        if profile_name not in active_profiles:
            duplicate_groups_by_profile[profile_name] = []
            continue

        grouped: dict[tuple, list[int]] = defaultdict(list)
        for household_id, signature in profile_map.items():
            # Ignore PV-profile matching for households that do not own PV.
            if profile_name == "pv_gen" and not has_pv_by_household.get(household_id, False):
                continue
            grouped[_signature_key(profile_name, signature)].append(household_id)
...
def clean_split(target_train_size: int = 150, mode: str = "strict_all") -> dict[str, list[int]]:
    '''
    ...
    '''
    _, groups_by_profile = _build_duplicate_groups(_collect_all_profiles(), mode=mode)
...
def _print_duplicate_report(groups_by_profile: dict[str, list[list[int]]], mode: str) -> None:
    active_profiles = _profile_names_for_mode(mode)
    profile_names = [name for name in groups_by_profile.keys() if name != "merged"]

    print(f"Duplicate groups by profile (mode={mode}):")
    for profile_name in profile_names:
        if profile_name not in active_profiles:
            print(f"- {profile_name}: ignored by mode")
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create duplicate-safe household train/test split.")
    parser.add_argument("--mode", choices=SPLIT_MODES, default="strict_all")
    parser.add_argument("--target-train-size", type=int, default=150)
    parser.add_argument("--quiet", action="store_true", help="Skip duplicate-group report and print only JSON split.")
    args = parser.parse_args()

    _, groups_by_profile = _build_duplicate_groups(_collect_all_profiles(), mode=args.mode)
    if not args.quiet:
        _print_duplicate_report(groups_by_profile, mode=args.mode)

    training, testing = _split_groups(groups_by_profile["merged"], target_train_size=args.target_train_size)
    split = {
        "training": training,
        "testing": testing,
    }
