...
drop_columns = ["household_id", "next_pv_gen"]

X_train, y_train = train.drop(columns=drop_columns), train["next_pv_gen"]
X_test, y_test = test.drop(columns=drop_columns), test["next_pv_gen"]
...
# save model
model.save_model("pv_gen_regressor.json")
