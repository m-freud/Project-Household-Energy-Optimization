# from SQLoader import transform_to_time_series
import datetime as dt

def period_to_epoch(period):
    seconds = period * 15 * 60
    base = dt.datetime(1899,12,30,0,0)
...
table_instructions = {
    ...
    "load": {
        "sheet_name": "Load",
        "rectangle": "A1:IQ97",
        "df_column_names": ["period"] + [i for i in range(1, 250+1)],
        "time_series": True,
        "transpose": False,
        "process": lambda df: transform_to_time_series(df, "period", "player_id", "load_kW")
    },
    "pv": {
        "sheet_name": "PV",
        "rectangle": "B1:IQ97",
        "df_column_names": ["player_id"] + [i for i in range(1, 96+1)],
        "transpose": True,
        "schema": f"""
