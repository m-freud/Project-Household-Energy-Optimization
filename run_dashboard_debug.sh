...
if "$WAIT_FOR_CLIENT"; then
	...
	exec "$PYTHON_BIN" -m debugpy --listen "$PORT" --wait-for-client -m streamlit run src/dashboard/dashboard.py
else
	echo "Starting dashboard with debugpy on port $PORT (not waiting for debugger attach)..."
	exec "$PYTHON_BIN" -m debugpy --listen "$PORT" -m streamlit run src/dashboard/dashboard.py
fi
