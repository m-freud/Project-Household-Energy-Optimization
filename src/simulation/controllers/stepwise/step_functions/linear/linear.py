...
def even_linear_policy(household: Household, scenario: Scenario|None = None) -> dict:
            ...
            if soc_deficit > 0:
                if remaining_steps <= 0:
                    ...
                    controls[f"{ev.name}_power"] = min(
                        (soc_deficit / (remaining_steps * RuntimeConfig.DURATION_TIMESTEP)) / ev.efficiency,
                        ev.max_charge,
                    )
    ...
    if household.bess:
        ...
        if soc_delta > 0:
            remaining_steps = max(deadline - household.current_timestep, 0)
            max_charge_by_deficit = soc_delta / (RuntimeConfig.DURATION_TIMESTEP * bess.efficiency)

            if remaining_steps <= 0:
                charge_power = min(bess.max_charge, max_charge_by_deficit)
            else:
                charge_power = min(
                    (soc_delta / (remaining_steps * RuntimeConfig.DURATION_TIMESTEP)) / bess.efficiency,
                    bess.max_charge,
            ...
            if remaining_steps <= 0:
                ...
                discharge_power = min(
                    (soc_surplus * bess.efficiency) / (remaining_steps * RuntimeConfig.DURATION_TIMESTEP),
                    bess.max_discharge,
...
def fast_charge_policy(household: Household, scenario: Scenario|None = None) -> dict:
    ...
    if household.ev1 and (household.ev1.at_home or household.ev1.at_charging_station):
        ...
        if household.ev1.soc < ev1_target_soc:
            required_power = (ev1_target_soc - household.ev1.soc) / (RuntimeConfig.DURATION_TIMESTEP * household.ev1.efficiency)
    ...
    if household.ev2 and (household.ev2.at_home or household.ev2.at_charging_station):
        ...
        if household.ev2.soc < ev2_target_soc:
            required_power = (ev2_target_soc - household.ev2.soc) / (RuntimeConfig.DURATION_TIMESTEP * household.ev2.efficiency)
    ...
    if household.bess:
        ...
        if soc_delta > 0:
            max_charge_by_deficit = soc_delta / (RuntimeConfig.DURATION_TIMESTEP * household.bess.efficiency)
            controls["bess_power"] = min(household.bess.max_charge, max_charge_by_deficit)
        elif soc_delta < 0:
            soc_surplus = -soc_delta
            max_discharge_by_surplus = (soc_surplus * household.bess.efficiency) / RuntimeConfig.DURATION_TIMESTEP
