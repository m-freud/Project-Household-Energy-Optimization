...
def price_aware_bess(household: Household, scenario: Scenario) -> dict:
    """
    Price-aware controller with active BESS optimization.

    - EVs: same behavior as price-aware linear 2 style (urgent => even, cheap => fast, else even).
    - BESS: cheap charge / expensive discharge / peak shaving by default,
      but urgency overrides to guarantee the deadline SOC target.
    """
    ...
    if q25 is not None and buy_price <= q25:
        max_charge_by_capacity = (bess.capacity - bess.soc) / (Config.DURATION_TIMESTEP * bess.efficiency)
        controls["bess_power"] = min(bess.max_charge, max(0.0, max_charge_by_capacity))
    elif q75 is not None and buy_price >= q75:
        max_discharge_by_soc = (bess.soc * bess.efficiency) / Config.DURATION_TIMESTEP
        controls["bess_power"] = -min(bess.max_discharge, max_discharge_by_soc, max_discharge_by_load)
    elif net_load > 0:
        max_discharge_by_soc = (bess.soc * bess.efficiency) / Config.DURATION_TIMESTEP
        controls["bess_power"] = -min(bess.max_discharge, max_discharge_by_soc, max_discharge_by_load)
